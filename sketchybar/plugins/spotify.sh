...
update_track() {
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME icon="" label="${TRACK} — ${ARTIST}" label.drawing=yes icon.drawing=on \
                        --set $NAME click_script="/usr/bin/open /Applications/Spotify.app" \


    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME icon="" label.drawing=off icon.drawing=off \
            --set $NAME click_script="/usr/bin/open /Applications/Spotify.app" \
